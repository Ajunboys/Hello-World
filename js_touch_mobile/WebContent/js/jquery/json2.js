...
JavaScript中没有直接串行化对象的内置方法，当然你可以自己写一个，
不过我还是强烈推荐你在这里偷个小懒，使用现成的组件，JSON的官方网站www.json.org
提供了对JavaScript对象实现JSON串行化的代码库—json2.js，你可以从这里获得它。
获得完毕json2.js文件后，你可以打开这个文件，在文件的前部分包含了相当大量的注释信息，
如果你的英文足够好，那么你可以省略我这一节，参考该文件的注释就可以了，如果作为程序员，
你已经看够了大片的字母，想看看我的汉字+字母，那你可以向下继续了。
简单的翻译下这个注释：
...
可选的filter参数是一个可过滤和转换结果的函数、它接受每个键和值，它的返回值用来替换源值。
如果它返回所接收的值，那么结果不会被改变。如果他返回undefined，则该成员会被删除。
例： 
代码:
...
上面的入门教程已经使你基本了解了json2.js的使用方法，这里关于该文件我就不再赘述了，
只是有一个小提示，如果你想简单的解析一个JSON文本，那么可以使用eval()函数，
改函数是JavaScript的内置函数，例如解析在JSON.stringify的案例中生成的JSON文本，可以使用：
代码:
var myE = eval('["e",{"pluribus":"unum"}]');
来获得对象myE。在prototype.js中还有另外的JSON
